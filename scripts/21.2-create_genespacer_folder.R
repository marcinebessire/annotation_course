...
# Load the annotation
annotation <- fread("/data/users/mbessire/annotation_course/maker/final2/filtered.genes.renamed.final.gff3", header = FALSE, sep = "\t")
...
top20_scaff <- fread("/data/users/mbessire/annotation_course/assembly/assembly_lja.fasta.fai", header = FALSE, sep = "\t") %>%
    select(V1, V2) %>%
    arrange(desc(V2)) %>%
    head(20)
...
longest_proteins <- "/data/users/mbessire/annotation_course/maker/final2/assembly.all.maker.fasta.all.maker.proteins.fasta.renamed.filtered.fasta"
