...
dat <- fread("/data/users/mbessire/annotation_course/genespacer/genespace/orthofinder/Results_Nov20/Comparative_Genomics_Statistics/Statistics_PerSpecies.tsv", header = T, fill = TRUE)
...
p <- ggplot(ortho_percent, aes(x = V1, y = as.numeric(perc), fill = species)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(y = "Count")
ggsave("orthogroup_percent_plot.pdf")


library(UpSetR)


orthogroups <- fread("/data/users/mbessire/annotation_course/genespacer/genespace/orthofinder/Results_Nov20/Orthogroups/Orthogroups.GeneCount.tsv")
